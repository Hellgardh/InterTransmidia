...
public class MercedesController_RB : MonoBehaviour
{
	public bool gamepad;
	...
	void Update ()
	{
		...
		if (Input.GetButtonDown("Jump")  && isGrounded|| Input.GetButtonDown("Gamepad_LB") && isGrounded)
		{
	...
	Vector3 GetInputs()
	{
        ...
        {
            inputs.x = Input.GetAxis("Gamepad_LStickX");
            inputs.z = Input.GetAxis("Gamepad_LStickY");
        }*/

		inputs = new Vector3(InputManager.M_Horizontal(), 0, InputManager.M_Vertical()); //InputManager: Script próprio de input para usar diferentes "controles" ao mesmo tempo

		inputs.x = InputManager.M_Horizontal();
		inputs.z = InputManager.M_Vertical();
